...
def compute_3d_conformer(mol: Mol, version: str = "v3") -> tuple[bool, Mol]:
    ...
    mol_path: Path = conformer_dir / f"{get_mol_id(mol)}.mol"
    if Path.exists(mol_path):
        mol = AllChem.MolFromMolFile(str(mol_path), removeHs=False)
    ...
    if conf_id != -1:
        ...
        conformer.SetProp("name", "Computed")
        conformer.SetProp("coord_generation", f"ETKDG{version}")
        AllChem.MolToMolFile(mol, str(mol_path), confId=conf_id)
        return (True, mol)
    return (False, mol)
